...
void ChatItemMsgFile::setSuccess()
{
    m_loading->hide();
    m_loadingMovie->stop();
    m_isSending = true;
...
void ChatItemMsgFile::setMessage(QString message, QString time, QSize allSize, MessageUtil::Bubble_Type bubbleType)
{
    m_msg = message;
...
void ChatItemMsgFile::paintEvent(QPaintEvent *event){
    ...
    if (m_bubbleType == MessageUtil::Bubble_She) {
        ...
        //内容
        ui->fileWidget->setGeometry(m_textLeftRect);
        ...
        painter.drawPolygon(points, 3);
        //内容
        ui->fileWidget->setGeometry(m_textRightRect);
